...
const NFTMint = ({ defaultAccount }) => {
    ...
    const updateEthers = async () => {
        try {
            ...
            //  剩下NFT還沒被mint的數量
            let leftNFT = await tempNFTContract.availableTokens()
            console.log(leftNFT[1].length)

            //  目前已被mint出來的數量
            let mintedSupply = await tempNFTContract.totalSupply()
            let normalNumber_mintedSupply = ethers.utils.formatUnits(`${mintedSupply}`, "wei")
            console.log(normalNumber_mintedSupply);

            //實際上的Supply = normalNumber_mintedSupply + leftNFT[1].length
            let totalSupply = Number(normalNumber_mintedSupply) + Number(leftNFT[1].length)
